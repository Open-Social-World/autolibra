...
class GeminiClient:
    ...
    def __init__(self):
        ...
        print(
            f"🛡️  Initializing Gemini {self.model_name} with safety threshold: BLOCK_NONE"
        )
...
def modify_behavior_clustering_template(num_metrics):
    ...
    try:
        ...
        new_matches = len(
            re.findall(f"Output\\s+{num_metrics}\\s+metrics\\.", modified_content)
        )

        if original_matches != 1 or new_matches != 1:
            print(
                f"⚠️  Unexpected number of pattern matches: original={original_matches}, new={new_matches}"
            )
        ...
        print(
            f"📝 Modified behavior clustering template to generate {num_metrics} metrics"
        )
        print(
            f"🔄 Changed from pattern 'Output N metrics.' to 'Output {num_metrics} metrics.'"
        )
