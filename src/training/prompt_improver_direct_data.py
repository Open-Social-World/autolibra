...
class AutoLibraPromptImprover:
    ...
    def __init__(
        ...
        self.turn_info_dir_template = paths_cfg.get(
            "turn_info_dir_template", "{base_dir}/repos/{repo}/{game}/turns/turn_{turn}/info"
        )
        self.turn_output_dir_template = paths_cfg.get(
            "turn_output_dir_template", "{base_dir}/repos/{repo}/{game}/turns/turn_{turn}/output"
        )
        ...
        print(f"🚀 AutoLibra Prompt Improver initialized with {len(self.prompting_strategies)} generalizable strategies")
        print(f"📍 Current turn: {self.current_turn}")
        print(f"📊 Loaded {len(self.metrics)} metrics")
        print(f"🎯 Loaded {len(self.eval_results)} evaluation results")
        print(f"🔧 Mode: Agent-only evolution with DIRECT DATA passing")
        print(f"⚡ Strategy: 8 generalizable prompting techniques + metric optimization")
        ...
        print(f"🎯 {len(strategy_names)} Generalizable Strategies: {', '.join(strategy_names)}")

        # Verify trajectory linking
        linked_count = sum(1 for result in self.eval_results if 'trajectory_data' in result)
        print(f"🔗 Successfully linked {linked_count}/{len(self.eval_results)} trajectories")
        print(f"📋 Template: Adaptive flexible prompt structure")
        print(f"🎮 Active target: repo={self.repo_name}, game={self.game_name}")
        print(f"🎯 NEW: Direct data passing for metric optimization")
    ...
    def _resolve_path(self, raw_path: str | Path) -> Path:
        ...
        context = {
            ...
            "base_dir": str(getattr(self, "base_dir", Path.cwd() / "prompt_optimization")),
    ...
    def show_rule_input_location(self):
        ...
        print(f"\n" + "="*60)
        print(f"📝 GAME CONTEXT SYSTEM INFORMATION")
        print(f"="*60)
        print(f"")
        print(f"🎮 The improver uses game context to generate optimized, domain-specific")
        print(f"   agent prompts tailored for maximum performance in the target game.")
        print(f"")
        print(f"📍 How it works: The system prompt includes detailed game information")
        print(f"   that informs the LLM about game mechanics, rules, and strategies.")
        print(f"")
        print(f"📋 This game context includes:")
        print(f"   - Available actions and game mechanics")
        print(f"   - Winning conditions and objectives")
        print(f"   - Game-specific rules and constraints")
        print(f"   - Object types and interactions")
        print(f"")
        print(f"✅ Result: The generated agent prompt is highly specific and optimized")
        print(f"   for this game domain, leading to better performance.")
        print(f"")
        print(f"🔧 For different games: Update the rule_input_section in the code")
        print(f"   with the new game's context information.")
        print(f"")
    ...
    def _setup_llm_client(self):
        ...
        if self.provider == "gemini":
            ...
            model_name = self.azure_model or os.getenv("AZURE_OPENAI_DEPLOYMENT") or os.getenv("AZURE_OPENAI_4O_MODEL")
            if not api_key or not endpoint or not model_name:
                raise ValueError("Azure provider selected but AZURE_OPENAI_API_KEY/ENDPOINT/DEPLOYMENT not set")
            client = AzureOpenAI(
                api_key=api_key,
    ...
    def _load_metrics(self) -> Dict[str, Any]:
        ...
        if not turn_info_dir.exists():
            raise FileNotFoundError(f"Could not find {turn_info_dir.name} directory at {turn_info_dir}")
        ...
        if not metrics_dirs:
            raise FileNotFoundError(f"Could not find any metrics directory in {turn_info_dir.name}")
    ...
    def _load_evaluation_results(self) -> List[Dict]:
        ...
        for subdir in turn_info_dir.iterdir():
            if subdir.is_dir():
                ...
                for pattern in ["llm_eval_results_*metrics.jsonl", "llm_eval_results*.jsonl", "eval_results*.jsonl"]:
                    eval_files.extend(subdir.glob(pattern))

        if not eval_files:
            raise FileNotFoundError(f"Could not find evaluation results file in {turn_info_dir.name}")
                ...
                if line.strip():
                    try:
                        ...
                        if 'instance_id' in result:
                            trajectory_data = self._load_trajectory_by_instance_id(result['instance_id'])
                            if trajectory_data:
    ...
    def _load_trajectory_by_instance_id(self, instance_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            trajectory_data = {
                ...
                'source_model': metadata.get('metadata', {}).get('source_model', 'unknown')
            }
            ...
            for timestamp in sorted(set(obs_files.keys()) | set(action_files.keys())):
                ...
                if 'observation' in step_data and isinstance(step_data['observation'], dict):
    ...
    def _init_prompting_strategies(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "direct_instruction": {
                ...
                "applicable_failures": ["unclear objectives", "rule confusion", "inaction", "goal confusion"],
                "applicable_successes": ["clear understanding", "focused execution", "rule compliance"],
            ...
            "few_shot": {
                ...
                "applicable_failures": ["unclear strategies", "no examples", "abstract concepts", "poor understanding", "lack of pattern recognition"],
                "applicable_successes": ["efficient solutions", "correct approaches", "clear reasoning", "systematic execution", "goal achievement"],
            ...
            "chain_of_thought_enhanced": {
                ...
                "applicable_failures": ["shallow analysis", "missing key insights", "rushed decisions", "incomplete reasoning"],
                "applicable_successes": ["structured thinking", "complete analysis", "careful evaluation"],
            ...
            "action_forcing": {
                ...
                "applicable_failures": ["idle behavior", "no actions", "static agent", "decision avoidance"],
                "applicable_successes": ["consistent progress", "decisive action", "goal-oriented behavior"],
            ...
            "constraint_awareness": {
                ...
                "applicable_failures": ["constraint violations", "impossible actions", "rule breaking", "invalid moves"],
            ...
            "progressive_hinting": {
                ...
                "applicable_failures": ["persistent confusion", "unable to start", "analysis paralysis", "decision paralysis"],
            ...
            "checklist_validation": {
                ...
                "applicable_failures": ["invalid moves", "rule violations", "hasty decisions", "action errors"],
            ...
            "negative_prompting": {
                ...
                "applicable_failures": ["irreversible mistakes", "task-ending actions", "critical resource destruction", "goal-blocking moves"],
        ...
        for result in self.eval_results:
            ...
            if is_successful:
                ...
                successful_pattern = {
                    ...
                    'successful_metrics': [metric for metric in self.metrics.keys()
                                         if result.get(metric.lower().replace(" ", "_").replace("-", "_"), 0) > 0],
                    'efficiency': 'High' if len(trajectory['steps']) < 50 else 'Moderate'
                }
                ...
                if failed_metrics_details:
                    failure_mode = {
                        'task': trajectory['task'],
                        'failure_type': 'Multiple metric failures' if len(failed_metrics_details) > 1 else failed_metrics_details[0]['metric'],
                ...
                if isinstance(first_obs, dict):
                    ...
                    if obs_text:
                        insights['observation_format_examples'].append({
                            'task': trajectory['task'],
                            'observation_sample': obs_text[:500] + "..." if len(obs_text) > 500 else obs_text
                        })
    ...
    def _analyze_failure_patterns(self, performance_analysis: Dict[str, Any]) -> Tuple[List[str], Dict[str, Dict[str, Any]]]:
        """Analyze failure patterns from reasoning text."""
        all_reasoning = performance_analysis['all_failure_reasoning']

        # Define failure pattern keywords (generalizable)
        failure_keywords = {
            'spatial_confusion': ['wrong direction', 'incorrect position', 'spatial', 'relative position',
                                'confused about location', 'misidentified position', 'wrong relative'],
            'rule_confusion': ['rule', 'confused', 'unclear rule', 'rule interpretation', 'misunderstood rule'],
            'goal_confusion': ['unclear goal', 'wrong objective', 'confused about goal', 'incorrect target'],
            'action_execution': ['failed to obtain', 'invalid action', 'parsing', 'action format'],
            'strategic_planning': ['no clear strategy', 'poor planning', 'inefficient', 'indirect path'],
            'inaction': ['idle', 'no action', 'static', 'waiting unnecessarily'],
            'constraint_violation': ['invalid move', 'blocked', 'boundary', 'constraint violated'],
            'resource_management': ['destroyed', 'lost critical', 'trapped essential', 'isolated important']
        }

        # Count occurrences
        pattern_counts = {pattern: 0 for pattern in failure_keywords}
        pattern_to_metric_details = {pattern: {'count': 0, 'metrics': [], 'examples': []}
                ...
                if score < 0 and reasoning:  # Negative score = failure
                    ...
                    for pattern, keywords in failure_keywords.items():
                        if any(keyword in reasoning_lower for keyword in keywords):
                            ...
                            if metric_name not in pattern_to_metric_details[pattern]['metrics']:
                                pattern_to_metric_details[pattern]['metrics'].append(metric_name)
    ...
    def _select_strategies_with_llm(self, performance_analysis: Dict[str, Any],
                                   failure_patterns: List[str],
                                   pattern_to_metric_details: Dict[str, Dict[str, Any]]) -> Tuple[List[str], str]:
        ...
        for metric, data in performance_analysis['metric_scores'].items():
            status = "🟢 GOOD" if data['all_positive'] else ("🔴 POOR" if data['all_negative'] else "🟡 MIXED")
        ...
        for pattern in failure_patterns[:5]:  # Top 5 patterns
            ...
            failure_summary.append(f"- {pattern}: {details['count']} occurrences affecting {', '.join(details['metrics'][:3])}")

        # Look for successful patterns to emphasize
        successful_metrics = [metric for metric, data in performance_analysis['metric_scores'].items()
...
Return a JSON object with:
{
  ...
  "rationale": "Brief explanation of why these strategies were chosen and how they work together"
        ...
        try:
            ...
            return self._fallback_strategy_selection(failure_patterns), "Fallback selection due to LLM error"
    ...
    def _fallback_strategy_selection(self, failure_patterns: List[str]) -> List[str]:
        ...
        pattern_to_strategies = {
            'spatial_confusion': ['direct_instruction', 'few_shot', 'chain_of_thought_enhanced'],
            'rule_confusion': ['direct_instruction', 'constraint_awareness', 'few_shot'],
...
""")
    ...
    def _generate_trajectory_examples(self, performance_analysis: Dict[str, Any]) -> str:
        ...
        if obs_examples:
            ...
            for i, example in enumerate(obs_examples[:3]):
                ...
                examples.append("```")
                examples.append("Key: Shows active rules first, then object positions with relative coordinates.")
        ...
        if successful_patterns:
            ...
            for i, pattern in enumerate(successful_patterns[:3]):  # Show more successful examples
                examples.append(f"\n**Success Pattern {i+1} - EXCELLENT PERFORMANCE:**")
                examples.append(f"- Task: {pattern['task']}")
                examples.append(f"- Completed in only {pattern['total_steps']} steps (Efficiency: {pattern['efficiency']})")
                examples.append(f"- Successfully achieved: {', '.join(pattern['successful_metrics'])}")
                examples.append(f"- **KEY SUCCESS FACTORS TO REPLICATE:**")
                examples.append(f"  * Clear goal identification and direct path planning")
                examples.append(f"  * Efficient spatial reasoning without unnecessary moves")
                examples.append(f"  * Strategic rule manipulation when needed")
                examples.append(f"  * Consistent action execution without hesitation")
                examples.append(f"- **EMULATE THIS:** {pattern['efficiency']} efficiency through focused, decisive action")
        ...
        if failure_modes:
            ...
            for i, failure in enumerate(failure_modes[:2]):  # Show fewer failure examples
                examples.append(f"\n**Challenge {i+1}:**")
                examples.append(f"- Task: {failure['task']}")
                examples.append(f"- Issue: {failure['failure_type']}")
                examples.append(f"- **Solution:** Apply successful patterns above to overcome this challenge")

                # Add specific metric failures
                if failure.get('failed_metrics'):
                    examples.append(f"- **Failed metrics:**")
                    for metric in failure['failed_metrics']:
                        examples.append(f"  * {metric['metric']}: {metric['score']} - {metric['reasoning'][:100]}...")

        # Add action failure analysis
        action_failures = trajectory_insights.get('action_failure_patterns', [])
        if action_failures:
            examples.append("\n**ACTION EXECUTION PATTERNS:**")
            examples.append(f"- Total action failures detected: {trajectory_insights.get('reasoning_quality_stats', {}).get('total_action_failures', 0)}")
            examples.append(f"- **Solution:** Ensure clear, decisive action selection with proper format")
            if action_failures:
                sample = action_failures[0]
                examples.append(f"- Example failure: Task '{sample['task']}' at step {sample['step_number']}")

        return "\n".join(examples)

    def _generate_failure_pattern_context(self, failure_patterns: List[str], pattern_to_metric_details: Dict[str, Dict[str, Any]]) -> str:
        ...
        for pattern in failure_patterns[:5]:  # Top 5 patterns
            ...
            if details['count'] > 0:
                ...
                context_lines.append(f"- Affects metrics: {', '.join(details['metrics'][:3])}")
                ...
                if pattern == 'spatial_confusion':
                    context_lines.append("- **Solution focus:** Enhance spatial reasoning with clear relative position tracking")
                elif pattern == 'rule_confusion':
                    context_lines.append("- **Solution focus:** Clarify rule explanations and provide concrete examples")
                elif pattern == 'action_execution':
                    context_lines.append("- **Solution focus:** Ensure clear action format and decisive selection")
                elif pattern == 'strategic_planning':
                    context_lines.append("- **Solution focus:** Implement structured planning with clear sub-goals")

                context_lines.append("")

        return "\n".join(context_lines)

    def _generate_agent_improvement(self, current_agent_code: str, performance_analysis: Dict[str, Any], failure_patterns: List[str], pattern_to_metric_details: Dict[str, Dict[str, Any]], selected_strategies: List[str], trajectory_examples: str) -> str:
        ...
        eval_results_sample = self.eval_results[:20] if len(self.eval_results) > 20 else self.eval_results
        ...
        for result in self.eval_results[:18]:  # Limit to 10 for space
            if 'trajectory_data' in result:
                ...
                trajectory_data.append({
                    ...
                    'first_30_steps': traj.get('steps', [])[:30],  # Include first 30 steps
                    'evaluation_scores': {k: v for k, v in result.items() if k.endswith('_score') or k in self.metrics}
                })
...
"""
    ...
    def improve_prompts(self) -> bool:
        ...
        print(f"\n" + "="*60)
        print(f"PHASE 1: TRAJECTORY-AWARE PERFORMANCE ANALYSIS")
        print(f"="*60)
        ...
        for metric, data in performance['metric_scores'].items():
            print(f"   {metric}: {data['average']:.2f} avg (negative: {data['all_negative']})")

        # Analyze failure patterns with detailed output
        print(f"\n🔍 FAILURE PATTERN ANALYSIS:")
        failure_patterns, pattern_to_metric_details = self._analyze_failure_patterns(performance)

        # Select strategies with detailed output  
        print(f"\n🎯 STRATEGY SELECTION:")
        selected_strategies, strategy_rationale = self._select_strategies_with_llm(performance, failure_patterns, pattern_to_metric_details)
        ...
        print(f"Generated {len(trajectory_examples.split('Example'))-1} trajectory examples")

        # PHASE 2: CONVERGENCE TO FINAL PROMPT
        print(f"\n" + "="*60)
        print(f"PHASE 2: PROMPT GENERATION WITH DIRECT DATA")
        print(f"="*60)
        ...
        print(f"🎯 Focus: METRIC OPTIMIZATION")
        ...
        if success:
            ...
            print(f"🎯 Created: {self.agent_prefix}{agent_number}Agent with direct data optimization")
            return True
        else:
            print(f"❌ Failed to save improvements")
            return False

    def _save_improved_prompts(self, agent_code: str, performance: Dict[str, Any]) -> bool:
    ...
    def run_full_improvement_cycle(self):
        ...
        if success:
            ...
            print(f"   2. Keep existing environment setup (no environment changes)")
            print(f"   3. Expect improved performance through metric optimization")
            print(f"\n📝 IMPORTANT - Key Innovation:")
            print(f"   - Agent now optimizes directly for metric scores")
            print(f"   - LLM analyzes raw data instead of pre-processed summaries")
            print(f"   - Better utilization of all three data sources")
            print(f"")
