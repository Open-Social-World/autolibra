...
feedback_grounding_semaphore = asyncio.Semaphore(
    20
)  # Limit to 20 concurrent feedback grounding tasks
...
class GeminiCompletions:
    ...
    async def parse(self, model: str, messages: list, response_format):
        ...
        print("    🚀 Using maximum 65K tokens for all requests")

        max_output_tokens = 65536

        print(
            f"    🌐 Calling Gemini API ({self.gemini_client.model_name}, max_tokens: {max_output_tokens:,})..."
        )
        ...
        while retry_count <= max_retries:
            try:
                ...
                self._log_token_usage(
                    response, operation_type, retry_count, success=True
                )
                break
                ...
                retry_count += 1
                print(
                    f"    ❌ API call failed (attempt {retry_count}/{max_retries}): {e}"
                )

                # Log failed attempt
                self._log_token_usage(
                    None, operation_type, retry_count - 1, success=False, error=str(e)
                )
        ...
        if response.candidates and len(response.candidates) > 0:
            ...
            if finish_reason == 2:  # MAX_TOKENS
                ...
                if self._has_valid_text(response):
                    print("    🔄 Found partial content, proceeding...")
                else:
                    print("    ❌ No content returned even with 65K tokens")
                    return await self._try_chunking_strategy(
                        full_prompt, response_format, schema
                    )
                ...
                raise RateLimitError(
                    f"Response blocked. Finish reason: {finish_reason}"
                )
        ...
        if not self._has_valid_text(response):
            ...
            raise RateLimitError("Response empty")
        ...
        try:
            ...
            if "FeedbackGroundingOutput" in format_name:
                print(
                    f"    🎯 Extracted {len(parsed_object.bullet_points)} bullet points"
                )
            elif "BehaviorClusteringOutput" in format_name:
            ...
            try:
                salvaged_response = self._salvage_partial_response(
                    safe_text, response_format, format_name
                )
                if salvaged_response:
                    print("    🚑 Successfully salvaged partial response")
            ...
            raise RateLimitError(
                f"JSON parsing failed after all recovery attempts: {e}"
            )
...
class GeminiClient:
    ...
    def __init__(self):
        ...
        print("🚀 Using maximum output token capacity: 65,536 tokens")
...
def load_existing_metrics(dataset_name, existing_metrics_path=None):
    ...
    if existing_metrics_path:
        ...
        print(
            f"📂 Loading existing metrics from specified path: {existing_metrics_path}"
        )
        ...
        elif os.path.exists(existing_metrics_path) and existing_metrics_path.endswith(
            "/metrics"
        ):
...
def filter_aspects_against_existing_metrics(aspects, existing_metrics):
    ...
    print(
        f"🔍 Filtering {len(aspects)} aspects against {len(existing_metrics)} existing metrics..."
    )
    ...
    for aspect in aspects:
        ...
        is_covered = any(
            # Exact match
            aspect_behavior == existing_behavior
            or
            # Check if aspect behavior is a substring of existing behavior
            aspect_behavior in existing_behavior
            or
    ...
    print(
        f"✂️  Filtered out {covered_count} aspects already covered by existing metrics"
    )
...
def modify_behavior_clustering_template_aggressive(
    num_new_metrics, existing_metrics=None, extra_enforcement=""
):
...
- FOLLOW THE EXACT COUNT: {num_new_metrics} metrics (not {num_new_metrics - 1}, not {num_new_metrics + 1})"""
                ...
                print("⚠️  Could not find any 'Output N metrics' pattern in template")
        ...
        if existing_metrics:
            existing_names = [
                metric.get("name", "Unknown") for metric in existing_metrics
            ]
        ...
        print(
            f"🤬 Modified template with AGGRESSIVE enforcement for {num_new_metrics} metrics"
        )
        if existing_metrics:
            print(
                f"🚫 Added explicit prohibition against {len(existing_metrics)} existing metric names"
            )
        ...
        if backup_path and backup_path.exists():
            try:
                ...
                print("🔄 Restored template from backup due to error")
...
def modify_behavior_clustering_template(num_new_metrics, existing_metrics=None):
    ...
    try:
        ...
        # Check if the pattern exists
        if not re.search(pattern, original_content):
            print("⚠️  Could not find 'Output N metrics.' pattern in template")
            print("📄 Template content around the end:")
        ...
        if modified_content == original_content:
            print("⚠️  No changes were made to the template")
            return template_path, backup_path
        ...
        print(
            f"📝 Modified behavior clustering template to generate {num_new_metrics} NEW metrics"
        )
        print("🔄 Added instruction to focus on filtered aspects only")
        if existing_metrics:
            print(
                f"🚫 Added explicit prohibition against {len(existing_metrics)} existing metric names"
            )
...
def restore_behavior_clustering_template(template_path, backup_path):
    ...
    if template_path and backup_path and backup_path.exists():
        try:
            ...
            if (
                not backup_content.strip()
                or "behavior_feedback_list" not in backup_content
            ):
                print("⚠️  Backup file seems invalid, skipping restore")
                return

            # Restore the template
            shutil.copy2(backup_path, template_path)
            print("🔄 Restored original behavior clustering template")
        ...
        print("⚠️  Cannot restore template - missing template_path or backup_path")
        if backup_path and backup_path.exists():
...
def save_token_usage_log(output_dir: Path, dataset_name: str, num_metrics: int):
    ...
    if TOKEN_USAGE["feedback_grounding"]["api_calls"] > 0:
        ...
        print(
            f"  • Input Tokens: {TOKEN_USAGE['feedback_grounding']['input_tokens']:,}"
        )
        print(
            f"  • Output Tokens: {TOKEN_USAGE['feedback_grounding']['output_tokens']:,}"
        )
    ...
    if TOKEN_USAGE["behavior_clustering"]["api_calls"] > 0:
        ...
        print(
            f"  • Input Tokens: {TOKEN_USAGE['behavior_clustering']['input_tokens']:,}"
        )
        print(
            f"  • Output Tokens: {TOKEN_USAGE['behavior_clustering']['output_tokens']:,}"
        )
...
def combine_old_and_new_metrics(existing_metrics, new_metrics):
    ...
    print(
        f"🔗 Combined {len(existing_metrics)} existing + {len(new_metrics)} new = {len(combined_metrics)} total metrics"
    )
...
async def main(
    dataset_name: str, num_metrics: int = 6, existing_metrics_path: str = None
) -> None:
    ...
    if num_new_needed <= 0:
        print(
            f"✅ Already have {num_existing} metrics, which meets or exceeds target of {num_metrics}"
        )
        print(
            "💭 Consider increasing target number or this run will just copy existing metrics"
        )
    ...
    if len(metric_training_instances) == 0:
        print(
            "⚠️  No training instances found! Check your dataset and annotation paths."
        )
        sys.stdout.flush()
        return
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if result and len(result) > 0:
                        if attempt > 0:
                            print(
                                f"    ✅ Feedback grounding succeeded on attempt {attempt + 1}"
                            )
                        return result
                    else:
                        if attempt < max_retries - 1:
                            print(
                                f"    ⚠️  Empty feedback grounding result on attempt {attempt + 1}, retrying..."
                            )
                            import time
                            ...
                            print(
                                f"    🚑 Using empty result after {max_retries} attempts"
                            )
                            return result
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"    ❌ Feedback grounding failed on attempt {attempt + 1}: {e}"
                        )
                        ...
                        print(
                            f"    ❌ Feedback grounding failed after {max_retries} attempts: {e}"
                        )
    ...
    print(
        f"✅ Feedback grounding completed. Processing {len(feedback_grounding_results)} results"
    )
    sys.stdout.flush()
    ...
    filtered_aspects = filter_aspects_against_existing_metrics(
        aspects, existing_metrics
    )
    ...
    if num_new_needed <= 0:
        ...
        print(
            "⚠️  No new aspects found after filtering - will return existing metrics only"
        )
        new_metrics = []
    else:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if (
                        ...
                        if attempt > 0:
                            print(
                                f"    ✅ Behavior clustering succeeded on attempt {attempt + 1}"
                            )
                        return result
                    else:
                        if attempt < max_retries - 1:
                            print(
                                f"    ⚠️  Empty behavior clustering result on attempt {attempt + 1}, retrying..."
                            )
                            import time
                            ...
                            print(
                                f"    🚑 Using empty clustering result after {max_retries} attempts"
                            )
                            return result
                    ...
                    if attempt < max_retries - 1:
                        print(
                            f"    ❌ Behavior clustering failed on attempt {attempt + 1}: {e}"
                        )
                        print("    ⏳ Waiting 2 seconds before retry...")
                        import time
                        ...
                        print(
                            f"    ❌ Behavior clustering failed after {max_retries} attempts: {e}"
                        )
        ...
        while len(all_new_metrics) < num_new_needed and attempts < max_attempts:
            ...
            print(
                f"🎯 Attempt {attempts}: Need {remaining_needed} more metrics (have {len(all_new_metrics)}/{num_new_needed})"
            )
            ...
            template_path_iteration, backup_path_iteration = (
                modify_behavior_clustering_template_aggressive(
                    remaining_needed,
                    existing_metrics,
                    prompt_modifier.format(remaining_needed=remaining_needed),
                )
            )

            try:
                behavior_clustering_results = await behavior_clustering_with_retry(
                    filtered_aspects
                )
                new_batch = behavior_clustering_results.metrics
                print(
                    f"📈 Generated {len(new_batch)} metrics in batch {attempts} (needed {remaining_needed})"
                )

                # Validate the count
                if len(new_batch) != remaining_needed:
                    print(
                        f"❌ AI FAILED: Asked for {remaining_needed} metrics, got {len(new_batch)}"
                    )
                    if len(new_batch) > remaining_needed:
                        print(
                            f"🚫 AI generated TOO MANY metrics ({len(new_batch)} > {remaining_needed})! This violates the MAXIMUM constraint!"
                        )
                        print(
                            f"🔪 Trimming to first {remaining_needed} metrics only..."
                        )
                        new_batch = new_batch[:remaining_needed]
                    else:
                        print(
                            f"📉 AI generated too few metrics ({len(new_batch)} < {remaining_needed})"
                        )
                    print(
                        f"🤬 AI is ignoring explicit instructions! Attempt {attempts}"
                    )
                ...
                restore_behavior_clustering_template(
                    template_path_iteration, backup_path_iteration
                )
        ...
        print(
            f"🎉 Final result: Generated {len(new_metrics)} unique new metrics after {attempts} attempts"
        )

        if len(new_metrics) < num_new_needed:
            print(
                f"💀 FAILURE: Only generated {len(new_metrics)} new metrics, wanted {num_new_needed}"
            )
            print(
                "🤬 The AI is consistently ignoring explicit metric count instructions!"
            )
            print(
                "💡 This indicates a fundamental problem with AI compliance to constraints"
            )
        else:
            print(
                f"🎯 SUCCESS: Generated exactly {len(new_metrics)} metrics as requested"
            )
    ...
    if actual_metrics_count != num_metrics:
        print(
            f"💀 CRITICAL FAILURE: Target was {num_metrics} metrics, but got {actual_metrics_count} metrics"
        )
        print(
            "🤬 This means the inductive approach FAILED to deliver the requested count"
        )
        print(
            f"📊 Breakdown: {len(existing_metrics)} existing + {len(new_metrics)} new = {actual_metrics_count} total"
        )
    else:
    ...
    print("📋 Feedback results: feedback_grounding_results.jsonl")
    print(
        f"📊 Final metrics: {len(existing_metrics)} existing + {len(new_metrics)} new = {actual_metrics_count} total"
    )
    if actual_metrics_count == num_metrics:
        print(f"✅ PERFECT: Delivered exactly {num_metrics} metrics as requested")
    else:
        print(
            f"❌ FAILED: Requested {num_metrics} but delivered {actual_metrics_count}"
        )
    sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Balrog Converter (Gemini Induction Version)"
    )
    parser.add_argument(
        "--filename",
        type=str,
