...
def run_command_with_env(cmd, description, env=None, cwd=None):
    ...
    try:
        result = subprocess.run(cmd, check=True, cwd=cwd, capture_output=True, text=True, env=env)
        if result.stdout:
            print(result.stdout)
        return True
    except subprocess.CalledProcessError as e:
...
def get_metrics_path_for_folder(folder_name):
    ...
    if "babaisai_1" in folder_name:
        ...
        print(f"⚠️  Warning: Could not determine babaisai level from folder name: {folder_name}")
        return None

def process_single_folder(folder_name, metrics_path=None, skip_convert=False, skip_babaisai=False, skip_llm_eval=False, api_key=None):
    ...
    if not skip_convert:
        ...
        cmd1 = [
            "python", "-m", "autolibra_core.datasets.convert_balrog_format",
            "--source", str(source_path)
        ]
        success = run_command(cmd1, f"Convert Balrog Format - {folder_name}", cwd=None)
        if not success:
            print(f"⚠️  Skipping remaining steps for {folder_name} due to conversion error")
            return False
    
    # Step 2: Process Balrog Babaisai
    if success and not skip_babaisai:
        cmd2 = [
            "python", "-m", "autolibra_core.datasets.balrog_babaisai",
            "--filename", f"gemini_babaisai/{folder_name}"
        ]
        success = run_command(cmd2, f"Balrog Babaisai Processing - {folder_name}", cwd=None)
        if not success:
            print(f"⚠️  Skipping LLM evaluation for {folder_name} due to babaisai processing error")
            return False
    ...
    if success and not skip_llm_eval:
        ...
        cmd3 = [
            "uv", "run", "python", "-m", "src.training.llm_as_a_judge_gemini",
            "--filename", f"gemini_babaisai/{folder_name}"
        ]
        ...
        if not final_metrics_path:
            ...
            if final_metrics_path:
                print(f"📊 Auto-detected metrics path based on babaisai level")
        ...
        if final_metrics_path:
            ...
            print(f"⚠️  No metrics path specified and could not auto-detect from folder name")
            return False
        
        # Run with modified environment
        success = run_command_with_env(cmd3, f"LLM as a Judge (Gemini) - {folder_name}", env=env)
    
    return success

def main():
    parser = argparse.ArgumentParser(description='Run the batch processing pipeline for Gemini Balrog data')
    parser.add_argument('--folders', nargs='+', help='Specific folders to process (if not specified, process all)')
    parser.add_argument('--metric-path', help='Path to the metrics directory for LLM evaluation (auto-detected based on babaisai level if not specified)')
    parser.add_argument('--skip-convert', action='store_true', help='Skip the convert_balrog_format step')
    parser.add_argument('--skip-babaisai', action='store_true', help='Skip the balrog_babaisai step')
    parser.add_argument('--skip-llm-eval', action='store_true', help='Skip the LLM evaluation step')
    parser.add_argument('--continue-on-error', action='store_true', help='Continue processing other folders even if one fails')
    parser.add_argument('--api-key', help='Gemini API key (required for LLM evaluation unless --skip-llm-eval is used)')
    ...
    summary_file = f"gemini_pipeline_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
