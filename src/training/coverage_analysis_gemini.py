...
coverage_analysis_semaphore = asyncio.Semaphore(20)  # Limit to 20 concurrent coverage analysis tasks
...
class GeminiCompletions:
            ...
            try:
                ...
                for field_name, field_info in fields.items():
                    if field_name.endswith('_reasoning'):
                        ...
                        properties[field_info.alias or field_name] = {"type": "integer", "enum": [-1, 0, 1]}
    ...
    async def parse(self, model: str, messages: list, response_format):
        ...
        if "AspectTraitsMatch" in format_name:
            print(f"    🔄 Using simplified native schema for AspectTraitsMatch")
            ...
            format_instruction += "Important: Use only these exact field names: aspect_0 and trait_0"
        ...
        print(f"    🏗️  Generated schema with {len(schema.get('properties', {}))} fields")
        generation_config = {
        ...
        for attempt in range(max_retries):
            ...
            print(f"    🌐 Calling Gemini API (native schema, max_tokens: 65536)... [Attempt {attempt + 1}]")
            ...
            try:
                ...
                # Log token usage
                self._log_token_usage(response, "coverage_breakdown", attempt, success=True)
                ...
                if not self._has_valid_text(response):
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"    🚑 Empty response after all retries, using minimal fallback")
                ...
                try:
                    ...
                    try:
                        ...
                        print(f"    🎯 Successfully parsed response into {type(parsed_object).__name__}")
                        return GeminiCompletion(parsed_object)
                    except Exception as validation_error:
                        # Handle Pydantic validation errors with smart repair for AspectTraitsMatch
                        if (hasattr(response_format, '__name__') and "AspectTraitsMatch" in response_format.__name__ 
                            and hasattr(self.gemini_client, '_current_trait_options')):
                            print(f"    🔧 Attempting smart repair for AspectTraitsMatch validation error...")
                            try:
                                repaired_data = _attempt_smart_repair(json_data, self.gemini_client._current_trait_options)
                                if repaired_data:
                                    repaired_object = response_format.model_validate(repaired_data)
                                    print(f"    🚑 Smart repair successful!")
                                    return GeminiCompletion(repaired_object)
                                else:
                                    print(f"    💔 Smart repair could not fix the data")
                            except Exception as repair_error:
                                print(f"    💔 Smart repair failed: {str(repair_error)[:50]}...")
                    ...
                    if attempt < max_retries - 1:
                        ...
                        print(f"    ❌ JSON parsing failed after all retries")
                        print(f"    🚑 All retries failed, using minimal fallback")
                        return self._create_minimal_response(response_format)
                        
                except Exception as e:
                    # Handle validation errors and other exceptions
                    print(f"    ⚠️  Validation failed on attempt {attempt + 1}: {str(e)[:100]}...")
                ...
                # Log failed attempt
                self._log_token_usage(None, "coverage_breakdown", attempt, success=False, error=str(api_error))
        ...
        print(f"    🚑 Unexpected end of retry loop, using minimal fallback")
        return self._create_minimal_response(response_format)
    ...
    def _create_minimal_response(self, response_format):
        ...
        if "AspectTraitsMatch" in class_name:
            ...
            try:
                    ...
                    if field_name.startswith('aspect_'):
                        ...
                        try:
                            ...
                            if hasattr(field_info.annotation, '__args__'):
                                ...
                                minimal_data[field_name] = field_info.annotation.__args__[0]
                            else:
                                # Fallback to a generic message
                                minimal_data[field_name] = "Unable to match aspect due to response limit"
                        except Exception as e:
                            print(f"    ⚠️  Could not extract literal for {field_name}: {e}")
                            minimal_data[field_name] = "Unable to match aspect due to response limit"
                ...
                raise ValueError(f"Cannot create minimal AspectTraitsMatch response: {e}")
            ...
            try:
                ...
                try:
                    ...
                    for field_name, field_info in fields.items():
                        if field_name.endswith('_reasoning'):
                            minimal_data_fallback[field_name] = "Unable to evaluate due to response limit"
                        else:
                            minimal_data_fallback[field_name] = 0
                    parsed_object = response_format.model_validate(minimal_data_fallback)
                    print(f"    🚑 Created minimal response for {class_name} using field names as fallback")
                    return GeminiCompletion(parsed_object)
            except Exception as e:
                raise ValueError(f"Cannot create minimal response for {class_name}: {e}")
...
def save_token_usage_log(dataset_name: str, results_file: str):
    ...
    if TOKEN_USAGE["coverage_breakdown"]["api_calls"] > 0:
        ...
        print(f"  • Input Tokens: {TOKEN_USAGE['coverage_breakdown']['input_tokens']:,}")
        print(f"  • Output Tokens: {TOKEN_USAGE['coverage_breakdown']['output_tokens']:,}")
...
def _attempt_smart_repair(json_data: dict, trait_options: list[str]) -> dict | None:
    ...
    try:
        ...
        for full_trait in trait_options[:-1]:  # Exclude "None..." option
            ...
            if (trait_name.lower() == trait_result.lower() or 
                trait_name.lower() in trait_result.lower() or 
                trait_result.lower() in trait_name.lower()):
                
                print(f"    🔧 Smart repair: '{trait_result[:30]}...' → '{full_trait[:30]}...'")
            ...
            if abbrev in trait_lower:
                ...
                for full_trait in trait_options[:-1]:
                    if full_trait.startswith(full_name + ":"):
                        print(f"    🔧 Smart repair (mapping): '{trait_result[:30]}...' → '{full_trait[:30]}...'")
                        repaired_data = json_data.copy()
                        repaired_data["trait_0"] = full_trait
                        return repaired_data
        
        # If no match found, suggest using "None..."
        print(f"    ⚠️  No repair possible for: '{trait_result[:30]}...', suggesting None")
        repaired_data = json_data.copy()
...
async def gemini_match_single_aspect_to_traits(client, aspect, traits):
    ...
    aspect_text = _sanitize_string(aspect.feedback) + ": " + _sanitize_string(aspect.behavior)
...
You must copy the text character-for-character. Do not abbreviate, summarize, or paraphrase."""
    ...
    try:
        ...
        completion = await client.beta.chat.completions.parse(
            ...
            messages=[
                {"role": "system", "content": "You must copy text exactly as provided. Do not abbreviate or paraphrase. Follow the literal constraints precisely."},
        ...
        return {
            "aspect_0": result.aspect_0,
            "trait_0": result.trait_0
        }
    finally:
        # Clean up
...
async def gemini_run_instance_coverage_eval(client, aspects, traits):
    ...
    print(f"  🟢 Processing {len(positive_aspects)} positive aspects against {len(positive_traits)} positive traits")
    print(f"  🔴 Processing {len(negative_aspects)} negative aspects against {len(negative_traits)} negative traits")
...
async def main(dataset_name: str, metric_path: str, results_file: str = "llm_eval_results_gemini.jsonl", test_instance_only: int = None) -> None:
    ...
    if test_instance_only is not None:
        if test_instance_only >= len(metric_training_instances):
            print(f"❌ Instance {test_instance_only} doesn't exist (only {len(metric_training_instances)} instances)")
            return
    ...
    if len(eval_results_raw) != len(metric_training_instances):
        print(f"⚠️  Warning: Evaluation results ({len(eval_results_raw)}) don't match training instances ({len(metric_training_instances)})")
    ...
    print(f"🔍 Debug - Evaluation processing:")
    ...
    print(f"   - Score matrix size: {len(eval_scoring)}x{len(eval_scoring[0]) if eval_scoring else 0}")
    if eval_scoring:
    ...
    print(f"🔍 Debug - Trait creation:")
    print(f"   - Traits created for {len(traits)} instances")
    if traits:
        print(f"   - Traits per instance: {len(traits[0])}")
        print(f"   - Sample trait ratings (first instance): {[t.rating for t in traits[0]]}")
    ...
    if total > 0:
        ...
        print(f"📊 Coverage: {covered}/{total} ({coverage_percent:.1f}% of feedback aspects covered)")
    else:
        print(f"📊 Coverage: {covered}/{total} (No aspects found)")
    
    if total_traits > 0:
        redundancy_percent = redundant/total_traits*100
        print(f"🔄 Redundancy: {redundant}/{total_traits} ({redundancy_percent:.1f}% of metrics unused)")
    else:
        print(f"🔄 Redundancy: {redundant}/{total_traits} (No traits found - check evaluation results)")
        print(f"⚠️  Warning: No traits were created. This suggests an issue with:")
        print(f"    - LLM evaluation results format")
        print(f"    - Metric loading/processing") 
        print(f"    - Trait creation logic")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Coverage Analysis with Unified Gemini - Phase 2")
    parser.add_argument(
        "--filename",
        type=str,
        required=True,
