...
class GeminiCompletions:
    ...
    def _extract_metrics_from_response_format(self, response_format):
        ...
        for field_name, field_info in fields.items():
            ...
            print(f"  🔍 Checking field: {field_name} -> corresponding reasoning field: {corresponding_reasoning_field} -> is score field: {is_score_field}")
            ...
            if is_score_field:  # This is a score field
                ...
                try:
                    ...
                    metric_explanation = field_info.description or f"Evaluation of {metric_name}"
    ...
    async def parse(self, model: str, messages: list, response_format):
        ...
        result = await self._try_with_token_limit(full_prompt, token_limit, response_format, gemini_response_format)
    ...
    def _manual_schema_fallback(self, response_format):
        ...
        try:
            ...
            reasoning_fields = [(name, info) for name, info in fields.items() if name.endswith('_reasoning')]
            score_fields = [(name, info) for name, info in fields.items() if not name.endswith('_reasoning')]
    ...
    async def _try_with_token_limit(self, full_prompt, token_limit, original_response_format, gemini_response_format):
        ...
        try:
            ...
            # Convert string enum values back to integers for score fields
            json_data = self._convert_enum_strings_to_ints(json_data, original_response_format)
        ...
        if "EvaluationResult" in class_name:
            ...
            try:
                ...
                for field_name, field_info in fields.items():
                    ...
                    alias = getattr(field_info, 'alias', field_name)
                    if field_name.endswith('_reasoning'):
                        minimal_data_fallback[alias] = "Unable to evaluate due to response limit - marking as not applicable"
                    else:
    ...
    with open(output_filename, "w") as f:
        for i, eval_result in enumerate(eval_results):
            ...
            for key, value in result_dict.items():
                if not key.endswith('_reasoning') and isinstance(value, str) and value in ["-1", "0", "1"]:
