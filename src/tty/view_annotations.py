        ...
        for annotation in trajectory_annotations.annotations:
            ...
            if isinstance(annotation.created_at, str):
                created_dt = datetime.fromisoformat(
                    annotation.created_at.replace("Z", "+00:00")
                )
            else:
            ...
            start_time = (
                annotation.span.start_time
                if annotation.span and annotation.span.start_time
                else "N/A"
            )
            end_time = (
                annotation.span.end_time
                if annotation.span and annotation.span.end_time
                else "N/A"
            )
...
def streamlit_main(annotations_dir: Path) -> None:
    ...
    df = df.sort_values("created_dt", ascending=False).drop_duplicates(
        subset=["instance_id", "agent_id", "annotator_id", "feedback"], keep="first"
    )
    ...
    if selected_annotator != "All":
        instance_options = (
            df[df["annotator_id"] == selected_annotator]["instance_id"]
            .unique()
            .tolist()
        )
    ...
    with col2:
        selected_instance = st.selectbox(
            "Select Instance", options=["All"] + sorted(instance_options)
        )
    ...
    for _, row in filtered_df.iterrows():
        with st.expander(
            f"Instance: {row['instance_id']} | Agent: {row['agent_id']} | {row['created_at']}",
            expanded=False,
        ):
