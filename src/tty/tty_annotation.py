...
class TTYAnnotator:
    ...
    def __init__(self, dataset_path: Path, annotation_path: Path, annotator_id: str, use_streamlit: bool = False):
    ...
    def run(self) -> None:
        ...
        if self.use_streamlit:
            ...
            if self.st.session_state.quit:
                ...
                self.st.balloons()
                self.st.success("Thank you for your annotations! You can close this window.")
                return
            ...
            self.st.write(f"Progress: {annotated_count}/{total_agent_instances} agent trajectories annotated")
            ...
            instance = self.dataset.get_instance_metadata(self.st.session_state.current_instance)
            trajectory = self.dataset.get_trajectory(self.st.session_state.current_instance, self.st.session_state.current_agent)
            trajectory_points = [
                (point, trajectory.get_data_at(idx))
                for idx, point in enumerate(trajectory.points)
            ]
            ...
            with col2:
                if self.st.button("Quit"):
                    self.st.session_state.quit = True  # Set quit state instead of raising KeyboardInterrupt
                    self.st.rerun()
            ...
            if self.st.session_state.trajectory_index < len(trajectory_points):
                ...
                self.st.markdown(f"### 👤 {self.st.session_state.current_agent}'s Trajectory")
                ...
                feedback = self.st.text_area("Please provide your feedback on the trajectory:", 
