...
class BalrogNLEDatasetLoader(BaseDatasetLoader):
    ...
    def download(self) -> None:
        download_github_folder(
            "balrog-ai",
            "experiments",
            "submissions/LLM/20241103_Claude-3.5-Sonnet/nle",
            ".data/balrog-nle",
        )
    ...
    def render(self) -> None:
        for csv_file, json_file in file_pairs(".data/balrog-nle/NetHackChallenge-v0"):
            ...
            for obs, action in zip(
                observations["Observation"].to_list(), actions["Action"].to_list()
            ):
                print(obs)
                print(action)
                # wait for user input
...
class BalrogMiniHackDatasetLoader(BaseDatasetLoader):
    ...
    def download(self) -> None:
        download_github_folder(
            "balrog-ai",
            "experiments",
            "submissions/LLM/20241103_Claude-3.5-Sonnet/minihack",
            ".data/balrog-minihack",
        )
...
class BalrogCrafterDatasetLoader(BaseDatasetLoader):
    ...
    def download(self) -> None:
        download_github_folder(
            "balrog-ai",
            "experiments",
            "submissions/LLM/20241103_Claude-3.5-Sonnet/crafter",
            ".data/balrog-crafter",
        )
...
class BalrogBabaIsAIDatasetLoader(BaseDatasetLoader):
    ...
    def download(self) -> None:
        download_github_folder(
            "balrog-ai",
            "experiments",
            "submissions/LLM/20241103_Claude-3.5-Sonnet/babaisai",
            ".data/balrog-babaisai",
        )
        ...
        for task in tasks:
            for csv_file, json_file in file_pairs(f".data/balrog-babaisai/{task}"):
                ...
                for obs, action in zip(
                    ...
                    active_rules, objects_on_the_map = text_removed_obs.split(
                        "Objects on the map:"
                    )
                    print(active_rules)
